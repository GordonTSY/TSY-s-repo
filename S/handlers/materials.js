...
function load_material(material_name, page, page_size, callback) {
    ...
    fs.readdir(
        "materials/" + material_name+"/"+access_material,
        function (err, files) {
			...
			var access_material=function(material_name){
	fs.readFile(
		"/users/user.json",
		function(err,contents){
			if(err){return;}
			contents.material_name.toString('utf8');
			var only_files = [];
			var path = "materials/" + material_name + "/"+;
					...
					async.forEach(
						...
						function (err) {
							if (err) {
								...
								var obj = { short_name: album_name,mfiles: mfiles };
								callback(null, obj);
							}
						}
					);
